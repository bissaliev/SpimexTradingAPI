...
async def test_get_expiries_return_correct_time(mock_datetime, return_value, expected):
    """Функция возвращает корректное время в секундах для кеша"""
...
class TestCacheUnit:
    ...
    async def test_work_cache(self, method, test_cache, mock_session, tradings_list):
        ...
        keys = [key async for key in test_cache.scan_iter("test-cache:*")]
        assert len(keys) == 1
        ...
        ttl = await test_cache.ttl(keys[0])
